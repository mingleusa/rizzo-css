...
- **`public/llms.txt`** — Official AI interface specification (plain text). Served at `/llms.txt` on the docs site so crawlers and LLMs can read it. **Version 3.0**; reflects actual naming (BEM with no prefix: `.btn`, `.navbar`, `.card`), 51 components, 6 blocks, 14 themes, five frameworks, and semantic tokens (`--background`, `--text`, `--accent`, etc.).
...
| **llms.json** | Structured summary for AI/scripts: architecture, tokens, 14 theme IDs, 51 component slugs, 6 block slugs, frameworks, aiRules. Matches llms.txt. |
...
- **llms.txt** and **llms.json** use spec version **3.0** and reference package version **0.0.67**.

...
const iconSvg = THEME_ICON_SVG[props.themeId] ?? null;
const sizedSvg = iconSvg
  ? iconSvg.replace(/width="16"/, `width="${props.size}"`).replace(/height="16"/, `height="${props.size}"`)
  : null;
</script>

<template>
  <span v-if="sizedSvg" :class="props.class" style="display: inline-flex; vertical-align: middle;" v-html="sizedSvg" />
</template>

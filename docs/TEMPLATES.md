...
| **Full**    | Full variant (`scaffold/vanilla/variants/full/`): Navbar, Settings, Footer, all component pages, `js/main.js`, CSS, fonts, sfx, icons. | Full variant (`scaffold/astro/variants/full/`): Navbar, Settings, Footer, all components, layout. | Full variant (`scaffold/svelte/variants/full/`): Navbar, Settings, Footer, all components, layout. | Full variant (`scaffold/react/variants/full/`): router, Docs/Blocks/Themes, Navbar, Settings, Footer, all components. | Full variant (`scaffold/vue/variants/full/`): router, Docs/Blocks/Themes, Navbar, Settings, Footer, all components. |
...
- **React:** Base is `scaffold/react/base/` (Vite + React). Variant overlays: `scaffold/react/variants/full/` (site clone with router, Docs/Blocks/Themes). CLI copies base then overlay when Full is selected, plus chosen components.
- **Vue:** Base is `scaffold/vue/base/` (Vite + Vue). Variant overlays: `scaffold/vue/variants/full/` (site clone with router, Docs/Blocks/Themes). Same flow as React.
...
The published tarball (`packages/rizzo-css` `"files"`) includes: **dist/** (rizzo.min.css, fonts/, sfx/), **bin/** (CLI), **scaffold/landing**, **scaffold/minimal** (snippet sources), **scaffold/astro** (base + variants + all 58 components), **scaffold/svelte** (base + variants + components), **scaffold/react** (base + variants/full + components), **scaffold/vue** (base + variants/full + components), **scaffold/vanilla** (index, components/, icons/, js/, variants/), **scaffold/config** (fonts.ts), **scaffold/shared** (navbar-vanilla.html), **scaffold/utils** (theme.ts). PrepublishOnly runs `build:css`, `copy-scaffold.js`, and `prepare-vanilla-scaffold.js` so dist and scaffolds are up to date before pack.
...
- **Docs / Dashboard:** Base + `scaffold/<framework>/variants/docs` or `scaffold/<framework>/variants/dashboard`. Applies to Vanilla, Astro, and Svelte; React and Vue use base only for Landing/Docs/Dashboard (full variant overlay exists for Full template).
- **Full:** `scaffold/<framework>/variants/full/` only (no base overlay). Available for Vanilla, Astro, Svelte, React, and Vue (React/Vue: router, Docs/Blocks/Themes layouts and views).

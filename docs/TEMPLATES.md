...
| Flow        | Choice              | Meaning |
|------------|---------------------|--------|
| **Create new** | CSS only \| Landing \| Docs \| Dashboard \| Full | CSS only = no components; Full = site clone; others = full framework + component picker (all 50 or pick). |
...
The published tarball (`packages/rizzo-css` `"files"`) includes: **dist/** (rizzo.min.css, fonts/, sfx/), **bin/** (CLI), **scaffold/landing**, **scaffold/minimal** (snippet sources), **scaffold/astro** (base + variants + all 50 components), **scaffold/svelte** (base + variants + components), **scaffold/react** (base + components), **scaffold/vue** (base + components), **scaffold/vanilla** (index, components/, icons/, js/, variants/), **scaffold/config** (fonts.ts), **scaffold/shared** (navbar-vanilla.html), **scaffold/utils** (theme.ts). PrepublishOnly runs `build:css`, `copy-scaffold.js`, and `prepare-vanilla-scaffold.js` so dist and scaffolds are up to date before pack.
...
- **add:** `--template css-only|landing|docs|dashboard|full` (same as create new). CSS only = no components; Full = site clone; others = component picker (all 50 or pick).

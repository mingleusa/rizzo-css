...
- **npm / pnpm / yarn:** `pnpm update rizzo-css` (or `npm update rizzo-css` / `yarn up rizzo-css`) to move to the latest patch. To jump to a specific version: `pnpm add rizzo-css@0.0.84`.
- **CDN:** Change the version in the URL (e.g. `unpkg.com/rizzo-css@0.0.84/dist/rizzo.min.css`). See [GETTING_STARTED.md](./GETTING_STARTED.md).

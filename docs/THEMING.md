...
- **ThemeIcon component** — Use the same icon elsewhere (e.g. theme doc pages, theme cards): **Astro** — `<ThemeIcon themeId="github-dark-classic" size={24} />`; **Svelte** — `<ThemeIcon themeId="github-dark-classic" size={24} />`. Both accept `themeId`, optional `size` (default 24), and optional `class`. **Vanilla** — Use the icon SVGs from the Icons section; map theme id to icon via `config/themes.ts` (each theme has an `iconKey`: owl, palette, flame, sunset, zap, shield, heart, sun, cake, lemon, rainbow, leaf, cherry, brush).

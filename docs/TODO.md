...
**Current state:** Single package **rizzo-css** (v0.0.18) with CSS, CLI, and scaffolds. **CLI:** `npx rizzo-css init` | `add` | `theme` | `help`. **Init:** framework (Vanilla / Astro / Svelte) → existing or new. **Existing** (or `add` command) → drop in CSS + hand-pick components. **New** → template (Vanilla: full or minimal; Astro/Svelte: minimal only) or **no template** (minimal base + hand-pick components), then package manager. Every scaffold includes LICENSE, README; Astro/Svelte include package.json and .env.example. **Add** = same as init → existing. Optional **rizzo-css.json**; **--yes**, **--template**, **--install**, **--write-config**. See [GETTING_STARTED](./GETTING_STARTED.md), [CLI](./CLI.md).

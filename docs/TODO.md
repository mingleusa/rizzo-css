...
- **CLI / npm package:** v0.0.85; `pnpm build:package` builds CSS, copies scaffold, prepares vanilla; `files` in package.json include bin, dist, scaffold/*.
...
- **Package:** Single **rizzo-css** (v0.0.85) — CSS, CLI, and scaffolds for Vanilla, Astro, Svelte, React, and Vue (all five have base scaffolds; React/Vue use Vite; Astro, Svelte, React, and Vue have base + variants including **Full** template). **Templates:** CSS only | Landing | Docs | Dashboard | Full (same for init and add); CSS only = no web pages or components; all **58 components** for other templates; we never overwrite existing files (snippets in RIZZO-SETUP.md). **Add** is for existing projects (select components or CSS only). Build: `pnpm build:package`. Docs: [docs/README.md](./README.md).

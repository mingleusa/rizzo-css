...
**Current state:** Single package **rizzo-css** (CSS + CLI + scaffolds) published; CDN works. CLI: `npx rizzo-css init` | `add` | `theme`. Init: **framework first**, then add to existing or create new. **Create new project** → full clone (prepare-* scripts). **Add to existing** → CSS + optional components. Scaffolds include theme persistence and toast; Vanilla includes Settings (`openSettings()`). Add auto-detects Svelte/Astro; `--path`, `--framework`. Versioning: [PUBLISHING](./PUBLISHING.md#versioning-strategy). React/Vue planned.

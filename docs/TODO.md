...
**Current state:** Single package **rizzo-css** (CSS + CLI + scaffold) is published; CDN (unpkg/jsDelivr) works. CLI: `npx rizzo-css init` | `add` | `theme`. **Init** uses interactive menus: project location, framework (**Vanilla JS** / Astro / Svelte — each with distinct CLI color: yellow, orange, orange-red), themes (multi-select), and (Astro/Svelte) components (multi-select). **Vanilla JS** gets `scaffold/vanilla/index.html` (theme switcher + sample components); all frameworks get the same CSS and component styles. **Add** auto-detects Svelte/Astro and copies to `static/css` or `public/css`; supports `--path` and `--framework`. Svelte and Astro components and docs live in-repo; framework switcher in place. JS utilities in `src/utils/`; versioning in [PUBLISHING](./PUBLISHING.md#versioning-strategy). React/Vue planned.

...
**Current state:** Single package **rizzo-css** (v0.0.21) with CSS, CLI, and scaffolds. **CLI:** `npx rizzo-css init` | `add` | `theme` | `help`. **Init:** framework (Vanilla / Astro / Svelte) → existing or new. **Existing** (or `add` command) → drop in CSS + hand-pick components. **New** → **Full** (everything) | **Minimal** (recommended) | **Manual** (pick components), then package manager. Every scaffold includes LICENSE, README; Astro/Svelte include package.json and .env.example. **Add** = same as init → existing. **rizzo-css.json** always written (new and existing); **--yes**, **--template**, **--install**, **--no-install**; interactive run prompts “Run install now?”. See [GETTING_STARTED](./GETTING_STARTED.md), [CLI](./CLI.md).

...
- **Build:** `pnpm build` — completes successfully (309 pages); tokens exported; CSS built; lint applied.
- **Bundle size:** `pnpm check:size` — package CSS within 450 kB budget.
- **Contrast:** `pnpm check:contrast` — all 14 themes meet WCAG AA.
- **Documentation:** Up to date — 53 components, six blocks, all framework tabs (Astro, Vanilla, Svelte, Vue, React) with framework-specific code blocks; Vue components index at `/docs/vue/components`.
- **CLI / npm package:** v0.0.74; `pnpm build:package` builds CSS, copies scaffold, prepares vanilla; `files` in package.json include bin, dist, scaffold/*.
- **Components & blocks:** 53 components across all five frameworks; six blocks with framework-specific snippets; live examples and code blocks accurate.
...
- **Carousel component** — *(done)* 53 components total; Astro, Svelte, React, Vue, Vanilla; BEM, prev/next, indicators; Storybook story; doc page and framework tabs.
...
- **Range Calendar** — *(done)* Date range selection; two clicks set start/end; `range-calendar-select` (detail.start, detail.end); Astro, React, Svelte, Vue, Vanilla; doc + framework tabs; 53 components total.
...
- **AI/LLM** — [public/llms.txt](../public/llms.txt) and [ai/llms.json](../ai/llms.json) reflect **53 components**, BEM naming, and button variants/sizes.
- **Framework parity** — All five frameworks (Astro, Svelte, React, Vue, Vanilla) have the same **53 components** and **6 blocks**. **Components:** canonical list in `src/config/reactComponents.ts` (`REACT_COMPONENT_SLUGS`); a11y `COMPONENT_SLUGS`; `docsNav`; Vue registry (`src/components/vue/registry.js`); React registry (`src/components/react/registry.tsx`); Svelte routes in `SvelteDocPage.svelte`; Astro pages `src/pages/docs/components/*.astro` (53); Vanilla pages `src/pages/docs/vanilla/components/*.astro` (53); React/Vue static paths from `reactDocPaths.ts`. **Blocks:** all 6 blocks (landing-hero, pricing, dashboard-01, docs-layout, login, signup) have framework-specific code for Astro, Svelte, React, Vue, and Vanilla in `src/config/blockCodeSnippets.ts`; block pages at `/blocks/*` use `FrameworkCodeTabs`. Docs sidebar includes a **Blocks** section (absolute links to `/blocks`, `/blocks/landing-hero`, etc.) so every framework’s docs expose blocks. CLI and vanilla scaffold must keep `COMPONENT_SLUGS` in sync for 53 component pages.
...
| **Carousel component** | *(done)* | 53 components total; Astro, Svelte, React, Vue, Vanilla; BEM, prev/next, indicators; Storybook; doc + framework tabs. |
...
- **React components** — *(done)* All **53 components** have full React implementations, live demos, and accurate React/TSX code blocks on each doc page. Same BEM and behavior as Astro and Svelte.
- **Vue components** — *(done)* All **53 components** with Vue implementations, live demos, and Vue SFC code blocks at /docs/vue; optional separate package in future.
...
| **A11y** | — *(matrix done)* |
| **Frameworks** | React: *(all 53 done)*; Vue: *(all 53 done)*; in-repo routes *(done)* |
...
| **Carousel** | *(done)* Sliding content with prev/next and indicators; 53 components total. |

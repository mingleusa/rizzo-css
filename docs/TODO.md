...
**Current state:** Single package **rizzo-css** (v0.0.17) with CSS, CLI, and scaffolds. **CLI:** `npx rizzo-css init` | `add` | `theme` | `help`. Supports **npm, pnpm, yarn, bun** (detection from lockfiles + `packageManager`; optional **rizzo-css.json** for targetDir, framework, packageManager). **Init:** framework (Vanilla / Astro / Svelte) → existing or new → (for new) **template** (Vanilla: full or minimal; Astro/Svelte: full-app or minimal) → themes → What to include? (Astro/Svelte) → package manager. **Templates:** Vanilla full = index + theme switcher, js/main.js, icons, README; minimal = HTML + CSS only. Astro/Svelte full-app = complete project scaffold; minimal = CSS + single HTML. **Add:** copies CSS; auto-detects framework; `--install-package` runs pm add. **Non-interactive:** `init --yes --framework vanilla|astro|svelte` and `--template`, `--install`, `--write-config`. Docs site and home: package manager tabs (npm, pnpm, yarn, bun) with copy for CLI and npm package install. See [GETTING_STARTED](./GETTING_STARTED.md), [CLI](./CLI.md), [CLI_PLANNING](./CLI_PLANNING.md).

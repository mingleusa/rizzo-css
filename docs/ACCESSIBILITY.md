...
- **Axe (WCAG)** — `tests/a11y/docs.spec.mjs`: entire main site (home, docs, every component page for **Astro, Vanilla, Svelte, React, and Vue**, **block pages** including Landing hero and Pricing, all theme pages). WCAG 2/2.1 A & AA; critical/serious only. Route list is built from `FOUNDATION_ROUTES`, `COMPONENT_SLUGS`, `BLOCK_ROUTES`, `THEME_SLUGS` in that spec; add new routes there as needed.

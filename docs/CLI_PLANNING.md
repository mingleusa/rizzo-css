...
> **Status**: Implemented. CLI ships in the **rizzo-css** package: `npx rizzo-css init` | `add` | `theme` | `help`. **Init:** framework (Vanilla / Astro / Svelte) → existing or new. **Existing** (or `add`): drop in CSS + hand-pick components. **New:** **Full**, **Minimal**, or **Manual** (per framework) → package manager (npm, pnpm, yarn, bun; detected suggested). **rizzo-css.json** (targetDir, framework, packageManager) always written for new and existing projects. **Add** uses config and detection; `--install-package` runs pm add. Non-interactive: `init --yes --framework vanilla|astro|svelte` and `--template`, `--install`, `--no-install`. See [CLI.md](./CLI.md).

...
| Command | Description |
|--------|-------------|
| `npx rizzo-css init` | First: **framework** (Vanilla / Astro / Svelte). Then: **Add to existing** (CSS + hand-pick components) or **Create new**. **New:** **Core** or **Manual** (per framework) → package manager. Package ships scaffold/vanilla, astro-core, svelte-core, astro/, svelte/. |
...
- [x] **Component selection:** Init (existing or new with no template) offers hand-pick components; **Core** gives the ready-made scaffold; **Manual** gives base + hand-pick (Astro/Svelte). Scaffold populated by `scripts/copy-scaffold.js` and `prepare-vanilla-scaffold.js` (run in prepublishOnly). Package ships scaffold/vanilla, astro-core, svelte-core, astro/, svelte/.

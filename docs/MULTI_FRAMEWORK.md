...
5. **Scaffold JS parity** — All three scaffolds (Vanilla, Astro, Svelte) include theme flash prevention and global toast (`showToast`, `removeToast`, `removeAllToasts`). Vanilla scaffold ships `js/main.js` with theme, Settings (`openSettings()`), tabs, modal, dropdown, accordion, search overlay, navbar mobile menu, and copy-to-clipboard. Tooltips use CSS only (:hover and :focus-within). Astro/Svelte package components include their own client scripts (e.g. Navbar mobile menu, Search with header + input + close button + sample results). The package scaffolds (`scaffold/astro/`, `scaffold/svelte/`) include **Navbar**, **Search**, and **Settings** (lightweight versions with full UI). **Full template** = site clone; **Landing/Docs/Dashboard** include all 56 components (Button, Badge, Card, Modal, Tabs, ThemeSwitcher, FormGroup, Alert, Toast, Dropdown, Navbar, Search, Settings, Accordion, CopyToClipboard, BackToTop). **Keyboard and click-outside:** All interactive overlays and menus are keyboard-navigable (Tab, Escape, arrows where applicable) and close when clicking outside: Search overlay, Settings panel, and Navbar mobile menu in all three frameworks.

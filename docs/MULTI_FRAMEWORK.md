...
- **npm:** `rizzo-css` — Single published package containing: **CSS** (`dist/rizzo.min.css`), **CLI** (`bin/rizzo-css.js`), and **scaffolds** (`scaffold/vanilla/`, `scaffold/astro-core/`, `scaffold/svelte-core/`, `scaffold/astro/`, `scaffold/config/`, `scaffold/svelte/`). Install with `pnpm add rizzo-css` and `import 'rizzo-css'`, or run `npx rizzo-css init`. Init first asks **framework** (Vanilla / Astro / Svelte), then **add to existing** (drop in CSS + hand-pick components) or **create new**. **Create new** → choose **Core** or **Manual** (Core = all 29 components; Manual picker has all pre-selected). All 29 components (including ThemeSwitcher) from `scaffold/astro/` and `scaffold/svelte/`; each scaffold has README-RIZZO.md, LICENSE-RIZZO, .gitignore, and (Astro/Svelte) package.json and .env.example. Navbar adds Search and Settings; Settings adds ThemeSwitcher; Toast adds Alert.

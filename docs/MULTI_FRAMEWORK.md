...
- **npm:** `rizzo-css` — Single published package containing: **CSS** (`dist/rizzo.min.css`), **CLI** (`bin/rizzo-css.js`), and **scaffolds** (`scaffold/vanilla/`, `scaffold/astro-core/`, `scaffold/svelte-core/`, `scaffold/astro/`, `scaffold/config/`, `scaffold/svelte/`). Install with `pnpm add rizzo-css` and `import 'rizzo-css'`, or run `npx rizzo-css init`. Init first asks **framework** (Vanilla / Astro / Svelte), then **add to existing** (same template choice: Minimal | Starter | Full) or **create new**. **Create new** → choose **Minimal**, **Starter**, or **Full** (Full = all 34 components or pick). **Add to existing** → same templates: Minimal/Starter = CSS + RIZZO-SETUP.md only; Full = CSS + component picker. All 34 components (including BackToTop, DocsSidebar, ThemeSwitcher, FontSwitcher, SoundEffects) from `scaffold/astro/` and `scaffold/svelte/`; each scaffold has README-RIZZO.md, LICENSE-RIZZO, .gitignore, and (Astro/Svelte) package.json and .env.example. Navbar adds Search and Settings; Settings adds ThemeSwitcher, FontSwitcher, SoundEffects; Toast adds Alert.

...
5. **Scaffold JS parity** — All three scaffolds (Vanilla, Astro, Svelte) include theme flash prevention and global toast (`showToast`, `removeToast`, `removeAllToasts`). Vanilla scaffold ships `js/main.js` with theme, Settings (`openSettings()`), tabs, modal, dropdown, and accordion inits; Astro/Svelte layouts include the scripts. The package scaffolds (`scaffold/astro/`, `scaffold/svelte/`) include **Navbar**, **Search**, and **Settings** (minimal versions); add them via the CLI when hand-picking components or copy full versions from the repo for full parity.

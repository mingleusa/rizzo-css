...
2. **Svelte components** — `src/components/svelte/` with 25 scaffold components (Button, Badge, Card, Divider, Spinner, ProgressBar, Avatar, Alert, Breadcrumb, Forms, CopyToClipboard, Tooltip, Pagination, Tabs, Accordion, Dropdown, Modal, Toast, Table, ThemeSwitcher, etc.) using the same BEM classes as Astro. ThemeSwitcher is included in the published scaffold (scaffold ships themes.ts and theme.ts so it works out of the box).
...
- **npm:** `rizzo-css` — Single published package containing: **CSS** (`dist/rizzo.min.css`), **CLI** (`bin/rizzo-css.js`), and **scaffolds** (`scaffold/vanilla/`, `scaffold/astro-core/`, `scaffold/svelte-core/`, `scaffold/astro/`, `scaffold/config/`, `scaffold/svelte/`). Install with `pnpm add rizzo-css` and `import 'rizzo-css'`, or run `npx rizzo-css init`. Init first asks **framework** (Vanilla / Astro / Svelte), then **add to existing** (drop in CSS + hand-pick components) or **create new**. **Create new** → choose **Core** or **Manual** (Core = all 33 components; Manual picker has all pre-selected). All 33 components (including BackToTop, DocsSidebar, ThemeSwitcher, FontSwitcher, SoundEffects) from `scaffold/astro/` and `scaffold/svelte/`; each scaffold has README-RIZZO.md, LICENSE-RIZZO, .gitignore, and (Astro/Svelte) package.json and .env.example. Navbar adds Search and Settings; Settings adds ThemeSwitcher, FontSwitcher, SoundEffects; Toast adds Alert.

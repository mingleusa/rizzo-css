...
2. **Svelte components** — `src/components/svelte/` with all 52 components (Button, Badge, Card, Divider, Spinner, ProgressBar, Avatar, Alert, Breadcrumb, Forms, CopyToClipboard, Tooltip, Pagination, Tabs, Accordion, Dropdown, Modal, Toast, Table, ThemeSwitcher, etc.) using the same BEM classes as Astro. ThemeSwitcher is included in the published scaffold (scaffold ships themes.ts and theme.ts so it works out of the box).

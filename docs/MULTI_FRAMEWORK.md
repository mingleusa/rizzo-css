...
5. **Scaffold JS parity** — All three scaffolds (Vanilla, Astro, Svelte) include theme flash prevention and global toast (`showToast`, `removeToast`, `removeAllToasts`). Vanilla scaffold ships `js/main.js` with theme, Settings (`openSettings()`), tabs, modal, dropdown, accordion, search overlay, navbar mobile menu, and copy-to-clipboard. Tooltips use CSS only (:hover and :focus-within). Astro/Svelte package components include their own client scripts (e.g. Navbar mobile menu, Search icon + overlay). The package scaffolds (`scaffold/astro/`, `scaffold/svelte/`) include **Navbar**, **Search**, and **Settings** (minimal versions); add them via the CLI when hand-picking components or copy full versions from the repo for full parity. **Keyboard and click-outside:** All interactive overlays and menus are keyboard-navigable (Tab, Escape, arrows where applicable) and close when clicking outside: Search overlay, Settings panel, and Navbar mobile menu in all three frameworks.

...
2. **Svelte components** — `src/components/svelte/` with all 53 components (Button, Badge, Card, Divider, Spinner, ProgressBar, Avatar, Alert, Breadcrumb, Forms, CopyToClipboard, Tooltip, Pagination, Tabs, Accordion, Dropdown, Modal, Toast, Table, ThemeSwitcher, etc.) using the same BEM classes as Astro. ThemeSwitcher is included in the published scaffold (scaffold ships themes.ts and theme.ts so it works out of the box).
...
5. **Scaffold JS parity** — All five scaffolds (Vanilla, Astro, Svelte, React, Vue) include theme flash prevention and global toast (`showToast`, `removeToast`, `removeAllToasts`). Vanilla scaffold ships `js/main.js` with theme, Settings (`openSettings()`), tabs, modal, dropdown, accordion, search overlay, navbar mobile menu, and copy-to-clipboard. Tooltips use CSS only (:hover and :focus-within). Astro/Svelte package components include their own client scripts (e.g. Navbar mobile menu, Search with header + input + close button + sample results). The package scaffolds (`scaffold/astro/`, `scaffold/svelte/`, `scaffold/react/base/`, `scaffold/vue/base/`) include **Navbar**, **Search**, and **Settings** (lightweight versions with full UI). **Full template** = site clone; **Landing/Docs/Dashboard** include all 53 components (Button, Badge, Card, Modal, Tabs, ThemeSwitcher, FormGroup, Alert, Toast, Dropdown, Navbar, Search, Settings, Accordion, CopyToClipboard, BackToTop). **Keyboard and click-outside:** All interactive overlays and menus are keyboard-navigable (Tab, Escape, arrows where applicable) and close when clicking outside: Search overlay, Settings panel, and Navbar mobile menu in all three frameworks.

**React:** All **53 components** have full React implementations, **working live demos**, and accurate React/TSX code blocks at `/docs/react/components/<slug>`. Same BEM and behavior as Astro and Svelte. **React doc pages** show Usage tabs for **Astro | Vanilla | Svelte | Vue | React** (default tab: React), so copy-paste code is available for every framework without leaving the React docs. **Astro reference pages** at `/docs/components/<slug>` also show all five framework tabs. Config: `src/config/reactComponents.ts`, `src/config/reactCodeSnippets.ts`, `src/config/astroCodeSnippets.ts`, `src/config/svelteCodeSnippets.ts`, `src/config/vanillaCodeSnippets.ts`, `src/config/reactDocPaths.ts`; registry: `src/components/react/registry.tsx`.

**Vue:** All **53 components** have Vue implementations, **working live demos**, and Vue SFC code blocks at `/docs/vue/components/<slug>`. Same BEM and behavior as Astro, Svelte, and React. **Vue doc pages** show Usage tabs for **Astro | Vanilla | Svelte | Vue | React** (default tab: Vue), so copy-paste code is available for every framework. Config: `src/config/vueCodeSnippets.ts`, `src/config/astroCodeSnippets.ts`, `src/config/svelteCodeSnippets.ts`, `src/config/vanillaCodeSnippets.ts`, `src/config/reactDocPaths.ts` (getVueDocStaticPaths); registry: `src/components/vue/registry.js`; demo: `src/components/vue/VueDocDemo.vue`.
...
- **npm:** `rizzo-css` — Single published package containing: **CSS** (`dist/rizzo.min.css`), **CLI** (`bin/rizzo-css.js`), and **scaffolds** (`scaffold/vanilla/`, `scaffold/astro/` (base + variants + components), `scaffold/svelte/` (base + variants + components), `scaffold/config/`). Install with `pnpm add rizzo-css` and `import 'rizzo-css'`, or run `npx rizzo-css init`. Init asks **framework** (Vanilla / Astro / Svelte / React / Vue), then **add to existing** or **create new** with template **CSS only** | **Landing** | **Docs** | **Dashboard** | **Full**. Full = site clone; others = all components or pick. All components (including BackToTop, DocsSidebar, ThemeSwitcher, FontSwitcher, SoundEffects, Skeleton, Switch, AlertDialog, Slider, Sheet, etc.) from `scaffold/astro/` and `scaffold/svelte/`; each scaffold has README-RIZZO.md, LICENSE-RIZZO, .gitignore, and (Astro/Svelte) package.json and .env.example. Navbar adds Search and Settings; Settings adds ThemeSwitcher, FontSwitcher, SoundEffects; Toast adds Alert.

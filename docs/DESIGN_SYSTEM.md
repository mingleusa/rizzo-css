...
- **One package, any framework** — Single **rizzo-css** (CSS, CLI, scaffolds). Templates: **CSS only** | **Landing** | **Docs** | **Dashboard** | **Full** (CSS only = no components; Full = site clone; others = all 52 or pick). [PUBLISHING](./PUBLISHING.md) for version/CDN and scaffold READMEs.

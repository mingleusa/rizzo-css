...
- **One package, any framework** — Single **rizzo-css** (CSS, CLI, scaffolds). Create new → **Core** or **Manual** (per framework). Add to existing (or `add` command) → drop in CSS + hand-pick components. [PUBLISHING](./PUBLISHING.md) for version/CDN and scaffold READMEs.

...
- **CDN** — unpkg and jsDelivr; pin with `.../rizzo-css@0.0.42/dist/rizzo.min.css`. Verify: `curl -I <url>` (200).
- **Svelte** — `/docs/svelte` (27 component pages). Scaffold ships 31 components (Core or Manual). React/Vue later.
...
|--------|----------------|
| `npx rizzo-css init` | Framework → add to existing or create new. **Existing** → CSS + components (add `<link>` yourself; CLI prints it). **New** → project location (**current directory** or **enter path or project name**; one prompt), then **Core** or **Manual**, then package manager. Install runs in the project directory. **--yes --path &lt;dir&gt;** scaffolds into that directory; **--yes** defaults to **core** template. Non-empty target directory prompts to continue. |
...
**Config:** **rizzo-css.json** `{ "targetDir", "framework", "packageManager", "theme" }`; unknown keys preserved. Init: **--yes**, **--path &lt;dir&gt;** (project directory), **--framework**, **--template**, **--package-manager**, **--install** / **--no-install**. Add: **--path** (CSS target dir), **--install-package**, **--no-snippet**, **--readme**, **--force**, **--vanilla-js**. **Templates:** Core = everything; Manual = same base, pick which components to include (all pre-selected). `npx rizzo-css help components` lists components and copy paths.

**Component dependencies:** Navbar adds Search and Settings; Settings adds ThemeSwitcher, FontSwitcher, SoundEffects; Toast adds Alert. Core includes these automatically; Manual shows e.g. "Navbar (adds Search, Settings)" in the picker. When you add **Settings** via the CLI (Astro or Svelte), the CLI also copies **scaffold/config/fonts.ts** into your project (Astro: `src/components/config/fonts.ts`; Svelte: `src/lib/config/fonts.ts`) so the font-pair dropdown works. List: `npx rizzo-css help components`. **Component counts:** Core = 31 components (all frameworks); Manual = 0–31 (all pre-selected by default; add/remove then confirm).
...
**First:** choose framework (Vanilla / Astro / Svelte). **Then:** add to existing or create new. **Existing** (or `add` command) → drop in CSS + hand-pick components; you must add the stylesheet `<link>` yourself (CLI prints the exact tag for your framework). **New** → project location (current directory or enter path/project name), then **Core** or **Manual**. Then package manager. **Add to existing** = CSS + hand-pick components. To only add CSS (with hand-pick flow):
...
| **Vanilla JS** | Same | Run `npx rizzo-css init` and choose Vanilla JS — **Core** or **Manual**. Core includes theme (with System), Settings panel, toast, and samples. Use [Vanilla component pages](/docs/vanilla/components) for copy-paste HTML and live demos. |
| **Astro**    | Same | Run `npx rizzo-css init` and choose Astro — **Core** (app + all components) or **Manual** (hand-pick; all pre-selected). Or `npx rizzo-css add` in an existing Astro project. [Docs](/docs/components). |
| **Svelte**   | Same | Run `npx rizzo-css init` and choose Svelte — **Core** (app + all components) or **Manual** (hand-pick; all pre-selected). Or `npx rizzo-css add` in an existing Svelte project. [Docs](/docs/svelte). Every component has Astro, Svelte, and Vanilla doc pages. |
...
│           ├── vanilla/      # Vanilla (Core / Manual: index, js, icons, components, README-RIZZO.md; no package.json)
...
See the [Components documentation](/docs/components) for the full list of 31 components (including ThemeSwitcher, FontSwitcher, SoundEffects), usage examples, and API details. Each component has a dedicated doc page with live examples.

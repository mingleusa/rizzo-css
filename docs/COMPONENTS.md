...
**Package:** Scaffolds: vanilla, astro-minimal, svelte-minimal, plus `scaffold/astro/`, `scaffold/utils/` (theme for ThemeSwitcher), and `scaffold/svelte/` (25 components including ThemeSwitcher). **Create new** → **Full**, **Minimal**, or **Manual** (minimal base + component picker; list shows which components add others, e.g. "Settings (adds ThemeSwitcher)"); **Add to existing** → CSS + hand-pick components. Full and Minimal include all required dependencies (Settings→ThemeSwitcher, Toast→Alert); run `npx rizzo-css help components` for the list. [GETTING_STARTED](./GETTING_STARTED.md) for setup.

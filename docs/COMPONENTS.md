...
- **Svelte** — Component files and docs at [/docs/svelte](/docs/svelte); use the **framework switcher** ("View as: Astro | Vanilla | Svelte | Vue | React") on any component or theme page. Use `npx rizzo-css init` and choose Svelte, then optionally add components from `scaffold/svelte/`.
- **React** — Full implementations and docs at [/docs/react](/docs/react); all 58 components have live demos and React/TSX code blocks. Same BEM and behavior as Astro and Svelte. Each React component page shows **Astro | Vanilla | Svelte | Vue | React** Usage tabs (default: React). Copy components from `src/components/react/` or use the tabs on [React](/docs/react), [Astro](/docs/components), or [Vue](/docs/vue) component pages.
- **Vue** — Full implementations and docs at [/docs/vue](/docs/vue); all 58 components have live demos and Vue/SFC code blocks. Same BEM and behavior as Astro, Svelte, and React. Each Vue component page shows **Astro | Vanilla | Svelte | Vue | React** Usage tabs (default: Vue). Copy components from the package scaffold or use the tabs on [Vue](/docs/vue), [Astro](/docs/components), or [React](/docs/react) component pages.
...
(All components in CLI and scaffold; component doc pages — some share a page e.g. Forms.) **Blocks** at [Blocks](/blocks): [Landing hero](/blocks/landing-hero), [Pricing cards](/blocks/pricing), [Dashboard](/blocks/dashboard-01), [Docs layout](/blocks/docs-layout), [Login](/blocks/login), [Sign up](/blocks/signup).
...
Every component has documentation and examples for **Astro**, **Svelte**, **React**, **Vue**, and **Vanilla** (HTML + same BEM). Each component page includes **Astro | Vanilla | Svelte | Vue | React** code tabs with complete, copy-paste examples so you can use the block that matches your project. Some pages use multiple code blocks (e.g. setup + usage) where needed.
...
|-----------|------------------|--------------|
| **Astro** | [/docs/components/&lt;name&gt;](/docs/components) (e.g. [/docs/components/button](/docs/components/button)) | Full Astro usage with **Astro | Vanilla | Svelte | Vue | React** code tabs (complete, copy-paste examples per framework), live demos, and props. |
...
| **React** | [/docs/react/components/&lt;name&gt;](/docs/react/components) (e.g. [/docs/react/components/button](/docs/react/components/button)) | React usage with **Astro | Vanilla | Svelte | Vue | React** code tabs (default: React), live demo, and same copy-paste examples as Astro pages. |
| **Vue** | [/docs/vue/components/&lt;name&gt;](/docs/vue/components) (e.g. [/docs/vue/components/button](/docs/vue/components/button)) | Vue usage with **Astro | Vanilla | Svelte | Vue | React** code tabs (default: Vue), live demo, and same copy-paste examples as Astro pages. |
...
Use the **framework switcher** ("View as: Astro | Vanilla | Svelte | Vue | React") at the top of any component or theme page to switch between framework docs.
...
4. **Snippet configs (for React/Vue doc pages)** — Add entries to `src/config/astroCodeSnippets.ts`, `src/config/svelteCodeSnippets.ts`, and `src/config/vanillaCodeSnippets.ts` so `/docs/react/components/<slug>` and `/docs/vue/components/<slug>` show accurate Astro, Svelte, and Vanilla snippets in their Usage tabs. React and Vue snippets come from `reactCodeSnippets.ts` and `vueCodeSnippets.ts`; defaults exist for any slug not in the override maps.
5. **CLI / scaffold (if shipping)** — Register the component in `packages/rizzo-css/bin/rizzo-css.js` (e.g. component list, copy paths, dependencies). Add scaffold files under `packages/rizzo-css/scaffold/vanilla/components/<slug>.html`, `scaffold/astro/`, and `scaffold/svelte/` as needed. Run `npx rizzo-css help components` to confirm.
6. **Svelte / Vanilla docs (optional)** — Add a Svelte doc page under `src/components/svelte/docs/pages/` and route in `src/pages/docs/svelte/[...slug].astro`; add or update Vanilla page under `src/pages/docs/vanilla/components/<slug>.astro`.
7. **Search** — Add an entry to the Algolia index or the static search data in `src/components/Search.astro` so the component appears in Cmd+K search. Run `node scripts/index-docs.js` (with Algolia env vars) to push updates; `componentPages` and `blockPages` in that script drive indexed component/block URLs.
8. **Tests** — If the component has an Astro doc page, add it to `tests/a11y/docs.spec.mjs`: use `COMPONENT_SLUGS` only if Vanilla and Svelte pages exist too; otherwise add the path to `ASTRO_ONLY_COMPONENT_ROUTES`. For new blocks add to `BLOCK_ROUTES`.

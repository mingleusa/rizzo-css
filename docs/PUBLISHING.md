...
- **CDN** — unpkg and jsDelivr; root URL serves CSS. Pin: `.../rizzo-css@0.0.68/dist/rizzo.min.css`. Verify: `curl -I <url>` (200).
...
- **Single package** — One **rizzo-css** (CSS, CLI, scaffolds). Templates: **CSS only** | **Landing** | **Docs** | **Dashboard** | **Full** (CSS only = no components; Full = site clone; others = all 52 or pick).
...
1. **Update version** (in both places if you keep them in sync):
   - `packages/rizzo-css/package.json` → `"version": "0.0.68"` (or next semver: patch/minor/major per [Versioning strategy](#versioning-strategy))
   - Optionally `package.json` at repo root (for the docs site)
   - Main `README.md` — update the npm badge so the version in the URL matches (e.g. `badge/npm-0.0.68-CB3837`)

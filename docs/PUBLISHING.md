...
The **rizzo-css** package (`packages/rizzo-css/`) ships built CSS, CLI (`init` / `add` / `theme` / `doctor` / `help`), and scaffolds (vanilla, astro-core, svelte-core, plus astro/ and svelte/ component templates). Init: framework → existing or new; both use the same template choice (Minimal | Starter | Full). [npm](https://www.npmjs.com/package/rizzo-css) · [Docs](https://rizzo-css.vercel.app).

## Features

- **NPM** — Package at `packages/rizzo-css/`; `pnpm build:css` → `dist/rizzo.min.css`. [Versioning strategy](#versioning-strategy) below.
- **CDN** — unpkg and jsDelivr; root URL serves CSS. Pin: `.../rizzo-css@0.0.54/dist/rizzo.min.css`. Verify: `curl -I <url>` (200).
- **Package contents** — Tarball includes `dist/`, `README.md`, `LICENSE`, `.env.example`, `bin/`, and under `scaffold/`: `minimal/`, `starter/`, `astro/`, `astro-core/`, `config/` (font pairs), `shared/` (sound script and default click.mp3/click.wav), `svelte/`, `svelte-core/`, `utils/` (theme for ThemeSwitcher), `vanilla/`). PrepublishOnly runs (from repo root): `lint:css:fix`, `build:css`, `copy-scaffold.js`, `prepare-vanilla-scaffold.js`.
- **Single package** — One **rizzo-css** (CSS, CLI, scaffolds). Create new = **Minimal** | **Starter** | **Full** (Full = all 34 or pick); Add to existing = same template choice (Minimal | Starter | Full).
...
1. **Commit all changes** — Everything that should be in this release: `scaffold/minimal/`, `scaffold/starter/`, `scaffold/vanilla/`, `scaffold/astro-core/`, `scaffold/svelte-core/`, `scaffold/astro/`, `scaffold/svelte/`, `scaffold/config/`, `scaffold/shared/`, `src/components/`, `src/components/svelte/`, CSS, CLI, docs (`docs/*.md`, `docs/planning/*.md`), and site pages (`src/pages/docs/`). The pack step uses the repo state at publish time (see [Keeping npm and GitHub in sync](#keeping-npm-and-github-in-sync)).
2. **Docs and site match current state** — Ensure all markdown docs (`docs/*.md`, `docs/planning/*.md`, `README.md`, `CONTRIBUTING.md`, `packages/rizzo-css/README.md`) and site content reflect the current project: version, component count (34), doc page count (29), icon counts (30 regular, 22 devicons, 52 total), CLI behavior, and scaffold contents. Run `pnpm build` and `pnpm test:a11y` to confirm the site builds and a11y tests pass.
3. **Version** — Bump `version` in `packages/rizzo-css/package.json` (and root `package.json` if you keep them in sync). Update the npm badge in the main `README.md` so the version in the badge matches (e.g. `badge/npm-0.0.54-CB3837`). Use [semver](https://semver.org/): patch for fixes/docs, minor for new features.
...
1. **Update version** (in both places if you keep them in sync):
   - `packages/rizzo-css/package.json` → `"version": "0.0.54"` (or next semver: patch/minor/major per [Versioning strategy](#versioning-strategy))
   - Optionally `package.json` at repo root (for the docs site)
   - Main `README.md` — update the npm badge so the version in the URL matches (e.g. `badge/npm-0.0.54-CB3837`)
...
- `scaffold/minimal`, `scaffold/starter`, `scaffold/astro`, `scaffold/config` (font pairs for Settings), `scaffold/utils` (theme utilities for ThemeSwitcher), `scaffold/svelte`, `scaffold/vanilla`, `scaffold/astro-core`, `scaffold/svelte-core`, `scaffold/shared`
- `README.md`
- `LICENSE` (MIT)

...
- **53 Svelte component pages** at `/docs/svelte` (with full Svelte examples; Icons, Navbar, Search, Settings, Theme Switcher, Font Switcher, Sound Effects include the same live standalone example as Astro, e.g. full Search component on the Search doc page).
...
- **React/Vue**: Astro integrations (`@astrojs/react`, `@astrojs/vue`); same pattern — `src/pages/docs/react/`, `src/components/react/`, and likewise for Vue. Scaffolds for React/Vue are Vite-based (see [planning/REACT_VUE_VITE_PLAN.md](./planning/REACT_VUE_VITE_PLAN.md)).

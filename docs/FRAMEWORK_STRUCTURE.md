# Framework structure (Vanilla JS, Astro, Svelte, React, Vue)
...
- **Framework switcher** — "View as: Astro | Svelte | Vanilla | React | Vue" (with framework icons) on component/theme doc pages; config in `src/config/frameworks.ts`.
- **58 Svelte component pages** at `/docs/svelte` (with full Svelte examples; Icons, Navbar, Search, Settings, Theme Switcher, Font Switcher, Sound Effects include the same live standalone example as Astro, e.g. full Search component on the Search doc page).
...
- **Adding React/Vue** — Same pattern: add to `frameworks.ts`, create `src/components/<framework>/` and `src/pages/docs/<framework>/`. Use **Astro integrations** (`@astrojs/react`, `@astrojs/vue`) for the docs site; React/Vue **scaffolds** are Vite-based. See [Adding a new framework](#adding-a-new-framework) below.

## Repository layout (high level)

- **`docs/`** — Markdown documentation. User-facing guides (GETTING_STARTED, DESIGN_SYSTEM, THEMING, etc.) and reference (COMPONENTS, CLI). See [docs/README.md](./README.md) for the full index.
- **`src/`** — Astro docs site and reference components (see below).
- **`packages/rizzo-css/`** — Published npm package: `bin/`, `dist/`, `scaffold/` (landing/, minimal/, vanilla/, astro/ with base + variants + components, svelte/ with base + variants + components, react/ with base + components, vue/ with base + components, config/, shared/, utils/).
...
- **All five frameworks** are organized consistently: **Astro** (`src/components/astro/`), **Vanilla** (docs at `src/pages/docs/vanilla/components/`; no component folder—HTML + BEM + scaffold `js/main.js`), **Svelte** (`src/components/svelte/`), **Vue** (`src/components/vue/`), **React** (`src/components/react/`).
- **Astro** design-system components live under `src/components/astro/`. Docs and shared site components (e.g. `CodeBlock`, `FrameworkCodeTabs`, `DocsSidebar`) stay at `src/components/` and `src/pages/docs/`.
- **Vanilla** has no in-repo component folder; same BEM and HTML as Astro. Doc pages at `src/pages/docs/vanilla/components/*.astro` include live demos with inline scripts where needed (accordion, collapsible, slider, tabs, table, copy-to-clipboard, toggle, etc.).
- **Svelte** docs live under `src/pages/docs/svelte/`; Svelte components live under `src/components/svelte/`.
- **React/Vue**: Astro integrations (`@astrojs/react`, `@astrojs/vue`); same pattern — `src/pages/docs/react/`, `src/components/react/`, and likewise for Vue. Scaffolds for React/Vue are Vite-based.
...
- **Framework switcher**: Uses `config/frameworks.ts` and shows “View as: Astro | Svelte | Vanilla | React | Vue” (with framework icons) on component/theme pages; each option links to the same path under that framework’s prefix.
- **Theme icons**: `config/themes.ts` defines theme id, label, icon, and preview colors; used by ThemeSwitcher and Navbar for consistent icons. **ThemeIcon** (Astro, Svelte, Vue, React) renders the same icon for a theme id (`themeId`, optional `size`, optional `class`); use on theme pages or in theme cards. Vanilla: use icon SVGs and map theme id to icon via `iconKey` in themes config.

## Adding a new framework

1. Add the framework to `src/config/frameworks.ts` (id, label, pathPrefix).
2. Create `src/components/<framework>/` and add components that use the same BEM classes. (Astro in `astro/`, React in `react/`, Vue in `vue/`, Svelte in `svelte/`. Vanilla uses HTML + BEM and doc pages at `src/pages/docs/vanilla/components/` with inline scripts for live demos.)

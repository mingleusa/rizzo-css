...
- **Vanilla JS** — No in-repo component folder; same BEM classes and HTML as Astro/Svelte. The **npm package and CLI** ship `scaffold/vanilla/` (`index.html` with inline theme flash prevention and `js/main.js` for theme, toast, settings, tabs, modal, dropdown, accordion; see `scaffold/vanilla/README.md`). The Vanilla scaffold does **not** use the npm package from node_modules: the CLI copies the built CSS into the project as `css/rizzo.min.css` and copies `js/main.js`; users can alternatively use a CDN link for CSS. Astro and Svelte scaffolds ship as `scaffold/astro-app/` and `scaffold/svelte-app/` (each has a README); optional components from `scaffold/astro/` and `scaffold/svelte/`. Add Navbar and Settings from this repo (or copy from scaffold components) for full parity with the docs site.
